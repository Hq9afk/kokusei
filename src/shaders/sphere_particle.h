...
float noise3(vec3 p) {
    return cnoise(vec4(p, 0.0), vec4(0.0));
}
...
float bassLevel(sampler2D tex) {
    return (sampleAudio(tex, 0.02) + sampleAudio(tex, 0.04) +
            sampleAudio(tex, 0.06) + sampleAudio(tex, 0.08)) * 0.25;
}

void main() {
    float bass = mix(bassLevel(u_audio_l), bassLevel(u_audio_r), a_grid.x);

    vec2 centered = a_grid - 0.5;
    vec3 p = vec3(centered * u_field_size, 0.0);
    vec3 np = vec3(centered * u_noise_frequency, 0.0);
    vec3 flow = u_noise_flow * u_time;

    float amplitude = u_noise_amplitude + bass * u_noise_audio_multiplier;
    vec3 displaced = p + amplitude * vec3(
        noise3(vec3(np.x + flow.x, np.y, flow.z + kDimOffset)),
        noise3(vec3(np.x + kDimOffset, np.y + flow.y, flow.z)),
        noise3(vec3(np.x, np.y + kDimOffset, flow.z)));

    float radius = u_sphere_radius + bass * u_radius_audio_multiplier;
    float dist = length(displaced);
    vec3 projected = displaced * (radius / max(dist, 0.0001));
    projected.z = abs(projected.z);
    vec3 finalPos = mix(displaced, projected, u_projection_factor);

    vec2 screen = finalPos.xy + u_screen * 0.5;
    vec2 ndc = vec2(screen.x / u_screen.x * 2.0 - 1.0,
                    1.0 - screen.y / u_screen.y * 2.0);
