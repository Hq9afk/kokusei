...
bool starward_init_egl(StarwardState &state, Renderer &renderer, EGLDisplay display,
...
void starward_handle_key_event(StarwardState &state, const KeyEvent &event) {
    ...
    case KeyKind::Left: {
        ...
        state.selected_index = (state.selected_index + kStarwardButtonCount - 1) %
    ...
    case KeyKind::Right: {
        int old = state.selected_index;
        state.selected_index = (state.selected_index + 1) % kStarwardButtonCount;
        trigger_highlight(state, old, state.selected_index);
        break;
    }
    case KeyKind::Text:
