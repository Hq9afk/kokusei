...
inline std::string extract_json_name(const char *value) {
    if (!value)
        return {};
...
inline void node_param_cb(void *data, int, uint32_t id, uint32_t index,
...
inline constexpr pw_node_events kNodeEvents = {
    .version = PW_VERSION_NODE_EVENTS,
    .info = node_info_cb,
    .param = node_param_cb,
};

inline void device_param_cb(void *data, int, uint32_t id, uint32_t, uint32_t,
                            const spa_pod *param) {
...
inline void device_info_cb(void *data, const pw_device_info *info) {
    auto *entry = static_cast<PwDeviceEntry *>(data);
...
inline constexpr pw_device_events kDeviceEvents = {
    .version = PW_VERSION_DEVICE_EVENTS,
    .info = device_info_cb,
    .param = device_param_cb,
};

inline int metadata_property_cb(void *data, uint32_t, const char *key,
...
inline constexpr pw_metadata_events kMetadataEvents = {
    .version = PW_VERSION_METADATA_EVENTS,
    .property = metadata_property_cb,
};

inline void registry_global_cb(void *data, uint32_t id, uint32_t,
...
inline void registry_global_remove_cb(void *data, uint32_t id) {
    auto *state = static_cast<PipewireState *>(data);
...
inline constexpr pw_registry_events kRegistryEvents = {
...
inline bool pipewire_init(PipewireState &state) {
    ...
    pw_registry_add_listener(state.registry, &state.registry_listener,
                             &pw_detail::kRegistryEvents, &state);

    return true;
}

inline int pipewire_fd(const PipewireState &state) {
...
inline PipewireChange pipewire_poll(PipewireState &state) {
    if (state.loop)
...
inline float pipewire_sink_level(const PipewireState &state, bool &muted) {
...
inline float pipewire_source_level(const PipewireState &state, bool &muted) {
...
inline void set_node_volume(PwNodeEntry &entry, float level) {
...
inline void set_node_muted(PwNodeEntry &entry, bool muted) {
    uint8_t buffer[512];
    spa_pod_builder b = SPA_POD_BUILDER_INIT(buffer, sizeof(buffer));
    spa_pod_frame f;
    spa_pod_builder_push_object(&b, &f, SPA_TYPE_OBJECT_Props, SPA_PARAM_Props);
...
inline void set_device_route_volume(pw_device *device_proxy,
                                    int32_t route_device, int32_t route_index,
                                    uint32_t channels, float level) {
...
inline void set_device_route_muted(pw_device *device_proxy,
                                   int32_t route_device, int32_t route_index,
                                   bool muted) {
    uint8_t buffer[1024];
    spa_pod_builder b = SPA_POD_BUILDER_INIT(buffer, sizeof(buffer));
...
inline void pipewire_set_node_volume(PipewireState &state, uint32_t id,
    ...
    if (pw_detail::resolve_device_route(state, entry, device_proxy,
                                        route_index))
        pw_detail::set_device_route_volume(device_proxy,
                                           entry.card_profile_device,
                                           route_index, entry.channels, level);
    else
        pw_detail::set_node_volume(entry, level);
...
inline void pipewire_set_node_muted(PipewireState &state, uint32_t id,
                                    bool muted) {
    auto it = state.nodes.find(id);
    if (it == state.nodes.end())
        return;
    ...
    if (pw_detail::resolve_device_route(state, entry, device_proxy,
                                        route_index))
        pw_detail::set_device_route_muted(
            device_proxy, entry.card_profile_device, route_index, muted);
    else
        pw_detail::set_node_muted(entry, muted);
    if (id == state.default_sink_id)
        state.sink_changed = true;
    else if (id == state.default_source_id)
        state.source_changed = true;
...
inline std::vector<const PwNodeEntry *>
pipewire_sinks(const PipewireState &state) {
...
inline std::vector<const PwNodeEntry *>
pipewire_sources(const PipewireState &state) {
    std::vector<const PwNodeEntry *> result;
    for (const auto &[id, entry] : state.nodes)
...
inline std::vector<const PwNodeEntry *>
pipewire_streams(const PipewireState &state, bool playback) {
    std::vector<const PwNodeEntry *> result;
    for (const auto &[id, entry] : state.nodes)
