...
#include "../../launcher/icon_theme.hpp"
...
struct TrayState {
    ...
    std::unordered_map<std::string, std::unique_ptr<sdbus::IProxy>> item_proxies;
...
template <typename T> inline std::optional<T> variant_get(const sdbus::Variant &v) {
    try {
        return v.get<T>();
...
inline TrayItem *find_item(TrayState &state, const std::string &key) {
...
inline void remove_item(TrayState &state, const std::string &key) {
    std::erase_if(state.items,
                  [&](const TrayItem &item) { return item.key() == key; });
    state.item_proxies.erase(key);
    state.menu_cache.erase(key);
    state.dirty = true;
}

inline void apply_item_props(TrayItem &item,
...
inline void watch_item_properties(TrayState &state, const std::string &key,
                                  sdbus::IProxy &proxy) {
    proxy.uponSignal("PropertiesChanged")
        .onInterface(kPropsIface)
        .call([&state, key](const std::string &,
...
inline void register_item(TrayState &state, const std::string &bus_name,
...
inline MenuEntry parse_menu_node(const DBusMenuLayout &node) {
    MenuEntry entry;
...
inline bool tray_init(TrayState &state) {
    using namespace tray_detail;
    try {
            ...
            ->addVTable(
                sdbus::registerMethod("RegisterStatusNotifierItem")
                    .implementedAs([&state](const std::string &service) {
                        std::string sender = state.watcher_object
                                                ->getCurrentlyProcessedMessage()
                                                .getSender();
                        std::string object_path =
                            service.starts_with('/') ? service
                                                     : "/StatusNotifierItem";
...
inline void tray_activate(TrayState &state, const TrayItem &item,
                          bool secondary) {
    ...
    it->second->callMethodAsync(secondary ? "SecondaryActivate" : "Activate")
        .onInterface(tray_detail::kItemIface)
...
inline void tray_menu_request(TrayState &state, const TrayItem &item,
                              std::function<void()> on_ready) {
    if (!item.has_menu)
        return;
    try {
        auto menu_proxy = sdbus::createProxy(
            *state.bus, sdbus::ServiceName{item.bus_name},
            sdbus::ObjectPath{item.menu_object_path});
        sdbus::IProxy *proxy_ptr = menu_proxy.get();

        proxy_ptr->callMethodAsync("AboutToShow")
            .onInterface(tray_detail::kMenuIface)
            .withArguments(int32_t{0})
            .uponReplyInvoke([](std::optional<sdbus::Error>) {});

        std::string key = item.key();
        auto owned_proxy = std::shared_ptr<sdbus::IProxy>(std::move(menu_proxy));
        proxy_ptr->callMethodAsync("GetLayout")
            .onInterface(tray_detail::kMenuIface)
            .withArguments(int32_t{0}, int32_t{-1}, std::vector<std::string>{})
            .uponReplyInvoke([&state, key, on_ready,
                             owned_proxy](std::optional<sdbus::Error> err,
                                         uint32_t,
                                         tray_detail::DBusMenuLayout layout) {
                if (err) {
                    klog("tray: GetLayout failed for %s: %s", key.c_str(),
                         err->getMessage().c_str());
                    return;
                }
                MenuEntry root = tray_detail::parse_menu_node(layout);
...
inline const std::vector<MenuEntry> *tray_menu_cached(const TrayState &state,
                                                       const std::string &key) {
    auto it = state.menu_cache.find(key);
    return it != state.menu_cache.end() ? &it->second : nullptr;
}

inline void tray_menu_event_clicked(TrayState &state, const TrayItem &item,
                                    int32_t entry_id) {
    try {
...
inline std::string tray_item_icon_path(const TrayItem &item) {
    if (item.icon_name.empty())
        return "";
    if (!item.icon_theme_path.empty()) {
        std::string candidate = item.icon_theme_path + "/" + item.icon_name + ".png";
