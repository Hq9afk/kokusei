...
inline std::vector<std::string> device_status_argv() {
    return {"nmcli", "-t", "-f", "DEVICE,TYPE,STATE,CONNECTION", "device",
            "status"};
}
inline std::vector<std::string> profile_argv() {
    return {"nmcli", "-t", "-f", "NAME,TYPE", "connection", "show"};
}
inline std::vector<std::string> quick_scan_argv() {
    return {"nmcli",  "-t",     "-f", "SSID,SECURITY,SIGNAL,IN-USE",
            "device", "wifi",   "list", "--rescan", "no"};
}
inline std::vector<std::string> scan_argv() {
    return {"nmcli",  "-t",     "-f", "SSID,SECURITY,SIGNAL,IN-USE",
            "device", "wifi",   "list", "--rescan", "yes"};
}
inline std::vector<std::string> connect_argv(const std::string &ssid,
                                             const std::string &password,
                                             bool saved) {
    if (saved)
        return {"nmcli", "-t", "connection", "up", "id", ssid};
    std::vector<std::string> argv = {"nmcli", "-t", "device", "wifi",
                                     "connect", ssid};
    if (!password.empty()) {
...
inline std::vector<std::string> disconnect_argv(const std::string &ssid) {
    return {"nmcli", "connection", "down", "id", ssid};
}

inline std::vector<std::string> forget_argv(const std::string &ssid) {
...
inline std::vector<std::string> connectivity_argv() {
    return {"nmcli", "networking", "connectivity", "check"};
}

inline void start_if_idle(AsyncProcess &proc, bool &running_flag,
...
inline void schedule_rescan(NetworkState &state, int ms) {
    state.next_rescan_at =
        std::chrono::steady_clock::now() + std::chrono::milliseconds(ms);
    state.rescan_scheduled = true;
}

inline void set_connectivity(NetworkState &state, const NetworkNotifyFn &notify,
...
inline void handle_connected_ssid_change(NetworkState &state,
...
inline bool network_init(NetworkState &state, sdbus::IConnection &bus) {
    try {
        state.nm = sdbus::createProxy(bus,
                                      sdbus::ServiceName{network_detail::kService},
                                      sdbus::ObjectPath{network_detail::kObjectPath});

        state.nm->uponSignal("PropertiesChanged")
            .onInterface(network_detail::kPropertiesIface)
            .call([&state](const std::string &iface,
                          const std::map<std::string, sdbus::Variant> &changed,
                          const std::vector<std::string> &) {
                if (iface != network_detail::kIface)
                    return;
                ...
                if (changed.count("State") || changed.count("ActiveConnections")) {
                    network_detail::start_if_idle(
                        state.device_proc, state.device_running,
                        network_detail::device_status_argv());
                    network_detail::start_if_idle(state.scan_proc,
                                                  state.scan_running,
                                                  network_detail::scan_argv());
                }
                if (changed.count("Connectivity")) {
                    network_detail::start_if_idle(
                        state.connectivity_proc, state.connectivity_running,
                        network_detail::connectivity_argv());
                }
            });
        ...
        network_detail::start_if_idle(state.device_proc, state.device_running,
                                      network_detail::device_status_argv());
...
inline void network_scan(NetworkState &state) {
    ...
    network_detail::start_if_idle(state.profile_proc, state.profile_running,
                                  network_detail::profile_argv());
    state.scanning = true;
}

inline void network_connect(NetworkState &state, const std::string &ssid,
                            const std::string &password) {
    if (state.connecting)
        return;
    ...
    state.connect_running =
        async_process_start(state.connect_proc,
                            network_detail::connect_argv(ssid, password,
                                                         state.connect_saved),
                            true) > 0;
}

inline void network_disconnect(NetworkState &state, const std::string &ssid) {
...
inline void network_forget(NetworkState &state, const std::string &ssid) {
    state.forget_ssid = ssid;
    state.forget_running = async_process_start(
                               state.forget_proc,
                               network_detail::forget_argv(ssid)) > 0;
}

inline void network_set_wifi_enabled(NetworkState &state, bool enabled) {
    if (!state.nm)
        return;
    try {
...
inline bool network_poll_device(NetworkState &state,
                                const NetworkNotifyFn &notify) {
    ...
    NetworkDeviceStatus st = network_parse_device_status(state.device_proc.buffer);
    ...
    if (st.ethernet_connected != was_connected) {
        if (st.ethernet_connected) {
            network_detail::start_if_idle(state.connectivity_proc,
                                          state.connectivity_running,
                                          network_detail::connectivity_argv());
        } else if (state.connected_ssid().empty()) {
            network_detail::set_connectivity(state, notify, "unknown");
        }
        if (state.init_done && notify) {
            if (st.ethernet_connected)
...
inline bool network_poll_profile(NetworkState &state) {
    ...
    network_detail::start_if_idle(state.quick_scan_proc,
                                  state.quick_scan_running,
                                  network_detail::quick_scan_argv());
    return true;
}

inline bool network_poll_quick_scan(NetworkState &state) {
    ...
    bool any_in_range = std::any_of(
        quick.begin(), quick.end(),
        [](const auto &kv) { return kv.second.in_range; });
    if (any_in_range)
        state.networks = std::move(quick);
    network_detail::start_if_idle(state.scan_proc, state.scan_running,
                                  network_detail::scan_argv());
    return true;
}

inline bool network_poll_scan(NetworkState &state, const NetworkNotifyFn &notify) {
    ...
    bool has_real = std::any_of(
        state.networks.begin(), state.networks.end(),
        [](const auto &kv) { return kv.second.in_range; });
    ...
    network_detail::schedule_rescan(state, next_ms);
    network_detail::handle_connected_ssid_change(state, notify);
    return true;
}

inline bool network_poll_connect(NetworkState &state, const NetworkNotifyFn &notify) {
    ...
    network_detail::schedule_rescan(state, 5000);
    network_detail::handle_connected_ssid_change(state, notify);
    return true;
}

inline bool network_poll_disconnect(NetworkState &state, const NetworkNotifyFn &notify) {
    ...
    network_detail::schedule_rescan(state, 3000);
    network_detail::handle_connected_ssid_change(state, notify);
    return true;
}

inline bool network_poll_forget(NetworkState &state) {
...
inline bool network_poll_connectivity(NetworkState &state,
                                      const NetworkNotifyFn &notify) {
    ...
    std::string text = network_detail::to_lower(
        network_detail::trim(state.connectivity_proc.buffer));
    network_detail::set_connectivity(state, notify,
                                     text == "none" ? "unknown" : text);
    return true;
}

inline bool network_tick(NetworkState &state,
    ...
    bool connected = !state.connected_ssid().empty() || state.ethernet_connected;
    if (connected) {
