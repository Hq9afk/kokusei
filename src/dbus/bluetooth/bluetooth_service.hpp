...
template <typename T>
inline std::optional<T> variant_get(const sdbus::Variant &v) {
    try {
        return v.get<T>();
...
inline void read_adapter_props(const InterfaceProps &props,
...
inline void merge_device_props(const InterfaceProps &props,
...
inline void merge_battery_props(const InterfaceProps &props,
                                BluetoothDeviceInfo &out) {
    if (!out.connected) {
        out.has_battery = false;
        out.battery_percent = 0;
...
inline BluetoothDeviceInfo *find_device(BluetoothState &state,
...
inline std::string connected_device_path(const BluetoothState &state) {
...
inline void notify_connection_change(BluetoothState &state,
...
inline void apply_managed_objects(BluetoothState &state,
...
inline bool bluetooth_init(BluetoothState &state, sdbus::IConnection &bus) {
...
inline sdbus::IProxy *bluetooth_detail_adapter(BluetoothState &state) {
...
inline void bluetooth_set_powered(BluetoothState &state, bool enabled) {
    sdbus::IProxy *adapter = bluetooth_detail_adapter(state);
    if (!adapter)
        return;
...
inline void bluetooth_start_discovery(BluetoothState &state) {
    sdbus::IProxy *adapter = bluetooth_detail_adapter(state);
    if (!adapter)
        return;
    try {
...
inline void bluetooth_stop_discovery(BluetoothState &state) {
    sdbus::IProxy *adapter = bluetooth_detail_adapter(state);
    if (!adapter)
        return;
    try {
...
inline void bluetooth_connect(BluetoothState &state,
...
inline void bluetooth_disconnect(BluetoothState &state,
                                 const std::string &device_path) {
    try {
        auto proxy =
            sdbus::createProxy(state.root->getConnection(),
...
inline void bluetooth_pair(BluetoothState &state,
                           const std::string &device_path) {
    if (BluetoothDeviceInfo *d =
            bluetooth_detail::find_device(state, device_path))
        d->connecting = true;
...
inline void bluetooth_forget(BluetoothState &state,
                             const std::string &device_path) {
    sdbus::IProxy *adapter = bluetooth_detail_adapter(state);
    if (!adapter)
        return;
...
inline void bluetooth_tick(BluetoothState &state,
                           const BluetoothNotifyFn &notify,
