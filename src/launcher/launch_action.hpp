...
inline std::string make_search_url(const std::string &text,
                                   const std::string &base) {
    std::string t = launch_action_detail::trim(text);
    if (t.empty())
        return "";
    return base + launch_action_detail::url_encode(t);
}

inline std::string normalize_url(const std::string &text) {
    std::string t = launch_action_detail::trim(text);
    if (t.empty())
        return "";
...
inline std::string resolve_web_target(const std::string &text,
                                      const std::string &base) {
...
inline bool launch_non_drun(LauncherMode mode, const std::string &query) {
    switch (mode) {
    case LauncherMode::Run: {
        std::string cmd = launch_action_detail::trim(query);
        if (cmd.empty())
            return false;
...
inline void launch_submenu_action(const SubmenuEntry &entry,
                                  VisitStore &visits) {
    switch (entry.action) {
...
inline void launch_drun_app(const DesktopEntry &entry, VisitStore &visits) {
