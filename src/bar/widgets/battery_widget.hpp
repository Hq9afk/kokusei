...
inline const float *battery_border_color(const UpowerState &u) {
    if (!u.present)
...
inline const char *battery_icon_glyph(const UpowerState &u) {
    if (!u.present)
...
inline std::string battery_label(const UpowerState &u) {
    if (!u.present)
        return "No Battery";
    if (u.full)
        return "Plugged in";
