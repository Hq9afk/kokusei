        ...
        case PanelClickKind::ToggleFlip: {
            ...
            if (region.tag == "fillmode") {
                ...
                int count = wallpaper_effective_column_count(
                    cfg, state.wallpaper_selected_monitor);
                count = std::clamp(count + (region.tag == "columnadd" ? 1 : -1),
                                   1, 6);
                updated
                    .wallpaper_column_counts[state.wallpaper_selected_monitor] =
                    count;
                on_commit(updated);
                ...
                if (is_default) {
                    ...
                    MonitorOverride &ov =
                        updated
                            .monitor_overrides[state.displays_selected_monitor];
                    if (region.tag == "osdenabled")
                        ov.osd = !ov.osd;
                    else if (region.tag == "notificationsenabled")
        ...
        case PanelClickKind::MonitorSelect:
            if (state.active_tab == SettingsTab::Displays) {
                state.displays_selected_monitor =
                    displays_monitor_from_tag(region.tag);
            } else {
    ...
    if (state.base.opacity > 0.0f) {
        ...
        case SettingsTab::Wallpaper: {
            state.wallpaper_grid_width =
                panel_x + panel_w - kPanelPadding - label_x;
            ...
            float grid_content_h = wallpaper_grid_content_height(
                state.wallpaper_picker, grid_inset_w);
            state.wallpaper_grid_height =
                std::min(grid_available_h,
        ...
        case SettingsTab::Displays: {
            ...
            if (!is_default) {
                auto it =
                    cfg.monitor_overrides.find(state.displays_selected_monitor);
