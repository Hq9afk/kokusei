...
inline std::string async_process_detail_resolve_path(const std::string &name) {
...
inline pid_t async_process_pid(const AsyncProcess &proc) {
    std::lock_guard<std::mutex> lock(proc.mutex);
    return proc.pid;
}

inline pid_t async_process_start(AsyncProcess &proc,
    ...
    pid_t pid = fork();
    if (pid < 0) {
        klog("async_process: fork failed");
        close(pipefd[0]);
        close(pipefd[1]);
        return -1;
    }
...
inline bool async_process_poll(AsyncProcess &proc) {
    std::lock_guard<std::mutex> lock(proc.mutex);
    if (proc.wake_fd >= 0) {
        uint64_t val;
...
inline pid_t async_process_cancel(AsyncProcess &proc) {
    pid_t killed = -1;
    int old_wake_fd = -1;
    {
        std::lock_guard<std::mutex> lock(proc.mutex);
