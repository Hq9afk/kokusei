...
inline RasterizedText surface_to_rgba(cairo_surface_t *surface, int width,
                                      int height) {
...
inline Texture make_texture_from_raster(const RasterizedText &raster,
...
inline PangoFontDescription *kokusei_font_description() {
    static PangoFontDescription *desc =
        pango_font_description_from_string(KOKUSEI_FONT);
    return desc;
}

inline PangoFontDescription *kokusei_font_description_small() {
    static PangoFontDescription *desc =
        pango_font_description_from_string(KOKUSEI_FONT_SMALL);
    return desc;
}

inline cairo_font_options_t *kokusei_font_options() {
...
inline std::string elide(const std::string &s, size_t max_chars) {
    if (s.size() <= max_chars)
        return s;
...
inline void font_ascent_descent(PangoFontDescription *desc, int &ascent,
                                int &descent) {
...
inline RasterizedText rasterize_text_with(const std::string &text,
...
inline RasterizedText rasterize_text(const std::string &text,
                                     int32_t scale = 1,
                                     int max_width_px = 0) {
...
inline RasterizedText rasterize_text_small(const std::string &text,
                                           int32_t scale = 1,
                                           int max_width_px = 0) {
