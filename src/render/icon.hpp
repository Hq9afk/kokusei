...
inline RasterizedText rasterize_icon(const std::string &codepoint_utf8,
                                     int32_t scale = 1) {
    RasterizedText result;
    icon_detail::IconFont &font = icon_detail::icon_font();
    if (!font.cairo_face)
        return result;

    uint32_t codepoint = icon_detail::decode_utf8_codepoint(codepoint_utf8);
    ...
    cairo_matrix_init_scale(&font_matrix, KOKUSEI_ICON_PX * scale,
                            KOKUSEI_ICON_PX * scale);
    cairo_matrix_t ctm;
