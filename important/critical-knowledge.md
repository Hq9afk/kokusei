...
- **A layer-shell overlay now stays mapped across same-output toggles, not destroying its surface on close.** Destroy-then-recreate left the compositor's layer stack stale until an unrelated event forced a recompute; see next entry.
- **Destroying and recreating a layer surface with the same namespace can leave it uncomposited until something else forces a layer-stack recompute.** Hyprland kept the reopened `overview`/`dashboard`/`settings` panel invisible until a workspace switch or the bar's own exclusive-zone change, neither related to the panel itself.
...
- **A local sdbus proxy destroyed right after firing an async call corrupts the shared connection it borrowed.** Cache one proxy per object path on the owning state instead of creating a throwaway per call.
