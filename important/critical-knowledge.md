...
- **An animation's `on_complete` that destroys the surface it's animating can fire mid-frame, inside the paint function's own `tick()` call.** Re-check the surface handle for validity immediately after `tick()`, before touching any EGL call that assumed it still exists.
...
- **A reference's per-output design choice can be a consequence of threading, not correctness.** kokusei is single-threaded, so one shared EGLContext suffices where noctalia needs one per output.
